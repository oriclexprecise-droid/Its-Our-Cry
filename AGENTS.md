...
- 当前待打包：
  ...
  4. 非原生角色包导入导出迁移：导入导出面板新增非原生角色包板块，默认全选导出多角色包（含模型、参考音频、参考文本、激活词、情绪）；导入支持多角色包与旧单角色包，撞名整包拒绝并提示；模型配置每行移除导入/导出按钮，仅保留添加、删除与激活词；本次提交 HASH_PENDING
  5. 状态：以上源码已改、已提交，桌面安装包尚未重建（release 仍为旧版）
