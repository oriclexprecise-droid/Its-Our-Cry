...
let qiniuDevUnlocked = false;

function initQiniuCloud() {
  const unlockBtn = document.getElementById("btn-qiniu-unlock");
  if (unlockBtn) unlockBtn.addEventListener("click", unlockQiniuDev);
  const lockBtn = document.getElementById("btn-qiniu-lock");
  if (lockBtn) lockBtn.addEventListener("click", lockQiniuDev);
  const saveBtn = document.getElementById("btn-qiniu-save");
  if (saveBtn) saveBtn.addEventListener("click", saveQiniuConfig);
  const testBtn = document.getElementById("btn-qiniu-test");
  if (testBtn) testBtn.addEventListener("click", testQiniuConnection);
  const refreshBtn = document.getElementById("btn-qiniu-refresh");
...
async function loadQiniuConfig() {
  const statusEl = document.getElementById("qiniu-status-line");
  try {
    const cfg = await api("/api/qiniu/config");
    if (cfg.configured) {
    ...
    const bucketEl = document.getElementById("qiniu-bucket");
    if (bucketEl) bucketEl.value = cfg.bucket || "";
    const domainEl = document.getElementById("qiniu-domain");
    if (domainEl) domainEl.value = cfg.domain || "";
    const regionEl = document.getElementById("qiniu-region");
    ...
    if (statusEl) { statusEl.textContent = "加载配置失败: " + e.message; statusEl.className = "status-text error"; }
  }
}

async function unlockQiniuDev() {
...
function lockQiniuDev() {
  ...
  const statusEl = document.getElementById("qiniu-dev-status");
  if (statusEl) { statusEl.textContent = "已退出开发者模式。"; statusEl.className = "status-text"; }
  refreshQiniuList();
}
...
async function saveQiniuConfig() {
  ...
  const payload = {
    ...
    access_key: (document.getElementById("qiniu-access-key") || {}).value || "",
    secret_key: (document.getElementById("qiniu-secret-key") || {}).value || "",
    bucket: (document.getElementById("qiniu-bucket") || {}).value || "",
    domain: (document.getElementById("qiniu-domain") || {}).value || "",
    region: (document.getElementById("qiniu-region") || {}).value || "z0",
    private: !!(document.getElementById("qiniu-private") || {}).checked
  };
  payload.access_key = payload.access_key.trim();
  payload.secret_key = payload.secret_key.trim();
  payload.bucket = payload.bucket.trim();
  payload.domain = payload.domain.trim();
  payload.new_password = payload.new_password.trim();
  if (!payload.access_key || !payload.secret_key || !payload.bucket || !payload.domain) {
    if (statusEl) { statusEl.textContent = "AccessKey、SecretKey、Bucket、域名都不能为空"; statusEl.className = "status-text error"; }
    return;
  }
  if (statusEl) { statusEl.textContent = "正在保存..."; statusEl.className = "status-text"; }
  try {
    await api("/api/qiniu/config", { method: "POST", body: JSON.stringify(payload) });
    const skEl = document.getElementById("qiniu-secret-key");
    if (skEl) skEl.value = "";
    const npEl = document.getElementById("qiniu-new-password");
    if (npEl) npEl.value = "";
    if (statusEl) { statusEl.textContent = "配置已保存，可点击测试连接验证。"; statusEl.className = "status-text success"; }
    loadQiniuConfig();
  } catch (e) {
    if (statusEl) { statusEl.textContent = "保存失败: " + e.message; statusEl.className = "status-text error"; }
  }
...
function fmtBilibili(url) {
  const v = (url || "").trim();
  if (!v) return "";
  if (/^https?:\/\//.test(v)) return v;
  if (/^space\.bilibili\.com\//.test(v)) return "https://" + v;
  return v;
}

async function refreshQiniuList() {
  const listEl = document.getElementById("qiniu-shared-list");
  const statusEl = document.getElementById("qiniu-list-status");
  if (!listEl) return;
  listEl.innerHTML = '<div class="model-empty">加载中...</div>';
  try {
    const data = await api("/api/qiniu/list");
    const items = data.items || [];
    if (!items.length) {
      listEl.innerHTML = '<div class="model-empty">暂无共享素材，解锁开发者模式后上传</div>';
      if (statusEl) { statusEl.textContent = ""; statusEl.className = "status-text"; }
      return;
    }
    listEl.innerHTML = items.map(it => {
      const bili = fmtBilibili(it.bilibili);
      return '<div class="webgal-map-row qiniu-item">'
        + '<span class="qiniu-item-main">' + esc(it.name) + '</span>'
        + '<span class="qiniu-item-meta">' + esc(it.category || "") + ' · ' + fmtBytes(it.size) + '</span>'
        + '<span class="qiniu-item-meta">' + (it.author ? esc(it.author) : "佚名") + (bili ? ' <a href="' + esc(bili) + '" target="_blank" rel="noopener">B站</a>' : '') + '</span>'
        + (it.description ? '<span class="qiniu-item-meta">' + esc(it.description) + '</span>' : '')
        + '<span class="qiniu-item-actions">'
        + '<button type="button" class="btn-secondary btn-small" data-download="' + esc(it.key) + '">下载</button>'
        + (qiniuDevUnlocked ? '<button type="button" class="btn-secondary btn-small" data-delete="' + esc(it.key) + '">删除</button>' : '')
        + '</span>'
        + '</div>';
    }).join("");
    listEl.querySelectorAll("[data-download]").forEach(btn => btn.addEventListener("click", () => downloadQiniuFile(btn.dataset.download)));
    listEl.querySelectorAll("[data-delete]").forEach(btn => btn.addEventListener("click", () => deleteQiniuFile(btn.dataset.delete)));
    if (statusEl) { statusEl.textContent = "共 " + items.length + " 个共享素材"; statusEl.className = "status-text"; }
  } catch (e) {
    listEl.innerHTML = '<div class="model-empty">云端尚未配置或连接失败</div>';
    if (statusEl) { statusEl.textContent = e.message; statusEl.className = "status-text error"; }
  }
}

async function downloadQiniuFile(key) {
  try {
    const res = await api("/api/qiniu/download", { method: "POST", body: JSON.stringify({ key }) });
    window.open(res.url, "_blank", "noopener");
  } catch (e) {
    showAlertModal("获取下载链接失败: " + e.message);
  }
}

async function deleteQiniuFile(key) {
  if (!(await showConfirmModal("确定删除这个共享素材吗？云端文件也会被删除。"))) return;
  const passwordEl = document.getElementById("qiniu-dev-password");
  try {
    await api("/api/qiniu/delete", { method: "POST", body: JSON.stringify({ key, dev_password: passwordEl ? passwordEl.value : "" }) });
    refreshQiniuList();
  } catch (e) {
    showAlertModal("删除失败: " + e.message);
  }
}

async function uploadQiniuFile() {
  const statusEl = document.getElementById("qiniu-upload-status");
  const fileInput = document.getElementById("qiniu-file");
  if (!qiniuDevUnlocked) { if (statusEl) { statusEl.textContent = "请先解锁开发者模式"; statusEl.className = "status-text error"; } return; }
  if (!fileInput || !fileInput.files || !fileInput.files[0]) { if (statusEl) { statusEl.textContent = "请选择文件"; statusEl.className = "status-text error"; } return; }
  const author = (document.getElementById("qiniu-author") || {}).value || "";
  if (!author.trim()) { if (statusEl) { statusEl.textContent = "请填写作者名（会展示在共享列表里）"; statusEl.className = "status-text error"; } return; }
  const fd = new FormData();
  const passwordEl = document.getElementById("qiniu-dev-password");
  fd.append("dev_password", passwordEl ? passwordEl.value : "");
  fd.append("category", (document.getElementById("qiniu-category") || {}).value || "二创素材");
  fd.append("author", author.trim());
  fd.append("bilibili", ((document.getElementById("qiniu-bilibili") || {}).value || "").trim());
  fd.append("description", ((document.getElementById("qiniu-description") || {}).value || "").trim());
  fd.append("file", fileInput.files[0]);
  if (statusEl) { statusEl.textContent = "正在上传..."; statusEl.className = "status-text"; }
  try {
    await api("/api/qiniu/upload", { method: "POST", body: fd });
    fileInput.value = "";
    if (statusEl) { statusEl.textContent = "上传成功，已加入共享列表"; statusEl.className = "status-text success"; }
    refreshQiniuList();
  } catch (e) {
    if (statusEl) { statusEl.textContent = "上传失败: " + e.message; statusEl.className = "status-text error"; }
  }
